
#######################################################São Paulo,Mato Grosso,Rio Grande do Sul,Rio Grande do Norte,Amazonas,Distrito Federal
library(readxl)
library(ggplot2)
library(reshape2)

data_path <- "Data_with_all_variables_ES.xlsx"
data_all <- read_excel(data_path, sheet = "Amazonas")


selected_cols <- c("cases", "tem", "pre", "dewpoint_tem", "Mean_sea_level_pressure", "min", "max", "rh", "wind")
data <- data_all[selected_cols]

colnames(data) <- c("Case", "Tavg", "Prec", "Tdew", "Mslp", "Tmin", "Tmax", "Rehu", "Wind")

data$Case <- log(data$Case + 1)  
colnames(data) <- c("ln(Case)", "Tavg", "Prec", "Tdew", "Mslp", "Tmin", "Tmax", "Rehu", "Wind")

cor_matrix <- cor(data, use = "pairwise.complete.obs", method = "spearman")
rownames(cor_matrix) <- colnames(cor_matrix) <- colnames(data)

cor_df <- melt(cor_matrix)

cor_df$abs_value <- abs(cor_df$value)

cor_df$stars <- cut(cor_df$abs_value,
                    breaks = c(-Inf, 0.25, 0.5, 0.75, Inf),
                    labels = c("", "*", "**", "***"),
                    include.lowest = TRUE)

cor_df$stars <- as.character(cor_df$stars)
cor_df$Var1 <- as.character(cor_df$Var1)
cor_df$Var2 <- as.character(cor_df$Var2)

cor_df$label <- ifelse(cor_df$Var1 < cor_df$Var2, 
                       sprintf("%.2f", cor_df$value), 
                       cor_df$stars)

matrix_height <- ncol(cor_matrix)


print(cor_df)


p <- ggplot(cor_df, aes(x = Var1, y = Var2)) +
  geom_tile(color = "black", fill = NA) +  
  geom_point(aes(size = abs_value, color = value), shape = 16) +  
  geom_text(aes(label = label), vjust = 0.5, color = "black", size = 5, na.rm = TRUE) +  
  scale_size_area(max_size = 25, guide = "none") +  
  scale_color_gradientn(  
    colors = c("#3278b5", "#408ac1", "#57a0ca", "#599fc9", "#77b5d6", "#97c8dc", "#b5d5e7", "#cde2f1", "#e2edf5", "#f4f9fa", "#fdf7f3", "#fde7da", "#ffd5c3", "#f9c0a4", "#f0ab87", "#e79072", "#DB725B", "#D05245", "#CF5447", "#BF3739"),
    values = scales::rescale(c(-1, -0.5, -0.25, 0, 0.25, 0.5, 0.75, 1)),
    limits = c(-1, 1)
  ) +
  theme_minimal() +
  theme(
    text = element_text(family = "Times New Roman"),
    plot.title = element_text(family = "Times New Roman"),
    axis.text.x = element_text(angle = 0, hjust = 0.5, size = 14, family = "Times New Roman"),
    axis.text.y = element_text(angle = 0, hjust = 0.5, size = 14, family = "Times New Roman"),
    axis.title.x = element_blank(),  
    axis.title.y = element_blank(), 
    plot.title.position = "plot",
    legend.position = "right", 
    legend.box.just = "center", 
    legend.title = element_text(size = 14, family = "Times New Roman"),
    legend.text = element_text(size = 12, family = "Times New Roman"),
    legend.key.height = unit(matrix_height * 5, "cm")
  ) +
  coord_fixed() +  
  guides(color = guide_colorbar(barheight = unit(matrix_height * 1.8, "cm"),
                                title.position = "top",
                                title.hjust = 0.5)) +
  theme(
    panel.grid.major = element_blank(),  
    panel.grid.minor = element_blank(),  
    panel.background = element_blank()   
  )


print(p)


ggsave(filename = "Correlation_Coefficients_Amazonas.tif",
       plot = p,
       device = "tiff",  
       width = 10,  
       height = 10,
       dpi = 300,
       units = "in")
